...
## 8. F-049 Mobile — crypto-parity contract DONE (2026-07)

F-049 is a native Android(Kotlin)+iOS(Swift) program (~23 d) that **can't be built here** (no gradle/kotlin; iOS can't read SMS). Started with the one high-leverage, verifiable-here linchpin: the **cross-language crypto contract** (D5/D27).
- **`docs/specs/crypto-parity-vectors.v1.json`** — canonical KDF + decrypt vectors the Android Kotlin `CryptoService` must reproduce byte-for-byte. Generated by `scripts/gen_crypto_vectors.swift` (mirrors `CryptoService`).
