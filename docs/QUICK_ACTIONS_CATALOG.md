...
## ✅ Currently Shipped (70 actions — v2.2 baseline + Phase 1 + Phase 2)
...
| [✅] | 48 | **Show Wi-Fi Password** | 🔧 | — | Reads the password for the currently connected Wi-Fi network from the macOS Keychain using `networksetup -getairportnetwork` + `security find-generic-password`. No admin needed for your own network. |
...
| [✅] | 86 | **Remove Duplicate Lines** | 📋 💻 | — | `pbpaste | awk '!seen[$0]++'` — deduplicates lines in clipboard preserving original order. Useful for cleaning up lists of IDs, URLs, or log entries. |
...
| [✅] | 91 | **Get Video File Info** | 🎨 | — | Reads a video file path from clipboard and runs `ffprobe` (or falls back to `mdls`) to display codec, resolution, frame rate, duration, and bitrate. |
...
| # | Action | Audience | 🔒 | Description |
|---|--------|----------|----|-------------|
| [✅] | 97 | **Lock Screen** | 🔧 | — | `pmset displaysleepnow` — instantly locks the screen. Faster than Ctrl+Cmd+Q for users who need keyboard-free locking. |
...
| [✅] | 113 | **Copy SSH Public Key** | 💻 | — | `cat ~/.ssh/id_ed25519.pub | pbcopy` — copies the default SSH public key (ed25519 or RSA) to clipboard. The single most-used action when setting up a new server or git remote. |
...
| **Total proposed** | **110** | |
| Already shipped (v2.2) | 15 | |
| Phase 1 shipped (v3.0) | 24 | |
| Phase 2 shipped (v3.0) | 29 | |
| **Total shipped** | **68** | |
...
### Phase 1 — High-value, low effort (24 actions) ✅ COMPLETE
All shell commands, no new Swift needed. Added 3 new categories (Developer, Files, Clipboard):
- ✅ Toggle Hidden Files, Restart Finder/Dock/Menu Bar, Show Public IP, Show Wi-Fi Password
...
- ✅ Lock Screen, Set Volume (Mute/50%), Remove .DS_Store, Show Largest Files
- ✅ Show Disk Usage, Eject All Disks, Copy SSH Public Key, Show All Listening Ports

### Phase 2 — Creator-focused (29 actions) ✅ COMPLETE
Added 2 new categories (Creative, Media). Optional deps (ffmpeg, qrencode) handled gracefully:
- ✅ All Creative Suite cache clears: FCP, Motion, DaVinci, Premiere, After Effects, Photoshop, Lightroom, Figma, Logic, Sketch
- ✅ Clipboard transforms: QR Code, URL encode/decode, Base64 encode/decode, Minify JSON, UPPERCASE/lowercase, Dedup Lines, Strip Formatting, SHA-256 Hash
- ✅ Media utilities: HEIC→JPEG, Optimise Images, Video Info, Extract Audio, Create GIF, Screenshot, Screen Recording, Resize to 1080p
...
*Phase 1 completed: 2026-06-01 | 24 actions implemented, tested, and validated*
*Phase 2 completed: 2026-06-01 | 29 actions implemented, tested, and validated*
*Next step: implement Phase 3 (Advanced developer tools) actions in ActionLibrary.swift*
